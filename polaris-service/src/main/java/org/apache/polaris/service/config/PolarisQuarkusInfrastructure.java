...
public class PolarisQuarkusInfrastructure {
  ...
  public RealmContextResolver realmContextResolver(
      @ConfigProperty(name = "polaris.context.realm-context-resolver.type") String type,
      @Any Instance<RealmContextResolver> realmContextResolvers) {
    return realmContextResolvers.select(Identifier.Literal.of(type)).get();
  }

  @Produces
  public FileIOFactory fileIOFactory(
      @ConfigProperty(name = "polaris.io.file-io-factory.type") String type,
      @Any Instance<FileIOFactory> fileIOFactories) {
    return fileIOFactories.select(Identifier.Literal.of(type)).get();
  }

  @Produces
  public MetaStoreManagerFactory metaStoreManagerFactory(
      @ConfigProperty(name = "polaris.persistence.metastore-manager.type") String type,
      @Any Instance<MetaStoreManagerFactory> metaStoreManagerFactories) {
    return metaStoreManagerFactories.select(Identifier.Literal.of(type)).get();
  }

  @Produces
  public RateLimiter rateLimiter(
      @ConfigProperty(name = "polaris.rate-limiter.type") String type,
      @Any Instance<RateLimiter> rateLimiters) {
    return rateLimiters.select(Identifier.Literal.of(type)).get();
  }

  @Produces
  public Authenticator<String, AuthenticatedPolarisPrincipal> authenticator(
      @ConfigProperty(name = "polaris.authentication.type") String type,
      @Any Instance<Authenticator<String, AuthenticatedPolarisPrincipal>> authenticators) {
    return authenticators.select(Identifier.Literal.of(type)).get();
  }

  @Produces
  public IcebergRestOAuth2ApiService icebergRestOAuth2ApiService(
      @ConfigProperty(name = "polaris.authentication.oauth2-service.type") String type,
  ...
  public TokenBrokerFactory tokenBrokerFactory(
      @ConfigProperty(name = "polaris.authentication.token-broker-factory.type") String type,
      @Any Instance<TokenBrokerFactory> tokenBrokerFactories) {
    return tokenBrokerFactories.select(Identifier.Literal.of(type)).get();
  }
